...
class MavenDataExtractor(private val workspace: Workspace) {
  ...
  private suspend fun extractFromFile(fileName: String): List<String> {
    return withContext(Dispatchers.IO) {
      val process =
        ProcessBuilder(bazelQuery.format(fileName).split(' ')).directory(workspace.path.toFile()).start().onExit().await()
      val xml = process.inputStream.bufferedReader().use { it.readText() }
      val fileLocation =
        Regex(regexPattern.format(fileName)).find(xml)?.let { it.groups[1]?.value } ?: throw RuntimeException("Failed to find file")
