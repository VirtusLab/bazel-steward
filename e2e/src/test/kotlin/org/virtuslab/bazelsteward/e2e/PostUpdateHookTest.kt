...
class PostUpdateHookTest : E2EBase() {
  ...
  @Test
  fun `Post hook update with maven install pin`(@TempDir tempDir: Path) {
    val project = "hook/pin"
    runBazelStewardWith(tempDir, project) {
      it.withMavenOnly(listOf("1.1.5"))
    }
    ...
    runBlocking {
      ...
      val oldDep = "io.arrow-kt:arrow-core:1.1.2"
      val newDep = "io.arrow-kt:arrow-core:1.1.5"
      val contents = localRoot.resolve("maven_install.json").toFile().readLines()
      contents.forEach { it shouldNotContain oldDep }
      contents.any { it.contains(newDep) } shouldBe true
